...
//指定のファンクションを devices
//に追加.もしPCI-PCIブリッジならセカンダリバスに対しScanBus()
...
WithError<uint64_t> ReadBar(Device &device, unsigned int bar_index) {
    if(bar_index >= 6) { return {0, MAKE_ERROR(Error::kIndexOutOfRange)}; }

    const auto addr = CalcBarAddress(bar_index);
    const auto bar = ReadConfReg(device, addr);

    // 32bit address
    if((bar & 4u) == 0) { return {bar, MAKE_ERROR(Error::kSuccess)}; }

    // 64bit address
    if(bar_index >= 5) { return {0, MAKE_ERROR(Error::kIndexOutOfRange)}; }
...
void InitializePCI() {
    ...
    for(int i = 0; i < pci::num_device; ++i) {
        ...
        Log(kDebug, "%d.%d.%d: vend %04x, class %02x.%02x.%02x, head %02x\n",
            dev.bus, dev.device, dev.function, vendor_id, class_code.base,
            class_code.sub, class_code.interface, dev.header_type);
    }
}