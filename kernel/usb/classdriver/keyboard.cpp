...
namespace usb {
	...
	Error HIDKeyboardDriver::OnDataReceived() {
		for (int i = 2; i < 8; ++i) {
			...
			if (std::find(prev_buf.begin(), prev_buf.end(), key) != prev_buf.end()) { continue; }
			NotifyKeyPush(key);
		}
		return MAKE_ERROR(Error::kSuccess);
	}

	void* HIDKeyboardDriver::operator new(size_t size) { return AllocMem(sizeof(HIDKeyboardDriver), 0, 0); }
