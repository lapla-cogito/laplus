...
fat::DirectoryEntry *FindCommand(const char *command,
    ...
    if(dir_cluster != 0 || strchr(command, '/') != nullptr) { return nullptr; }
...
Terminal::Terminal(Task &task, const TerminalDescriptor *term_desc)
    ...
    if(show_window_) {
        window_ = std::make_shared<ToplevelWindow>(
            ...
            screen_config.pixel_format, "laplusTerminal");
...
Rectangle<int> Terminal::InputKey(uint8_t modifier, uint8_t keycode,
    ...
    } else if(keycode == 0x51) { // 下矢印
        draw_area = HistoryUpDown(-1);
    } else if(keycode == 0x52) { // 上矢印
        draw_area = HistoryUpDown(1);
    }
...
void Terminal::ExecuteLine() {
    ...
    if(first_arg) {
        *first_arg = 0;
        do { ++first_arg; } while(isspace(*first_arg));
    }
    ...
    if(redir_char) {
        *redir_char = 0;
        ...
        while(isspace(*redir_dest)) { ++redir_dest; }
	...
	/*notermコマンド*/
        ...
        if(t.TimeZone == EFI_UNSPECIFIED_TIMEZONE) {
            ...
            PrintToFD(*files_[1], "%d-%02d-%02d %02d:%02d:%02d ", t.Year,
                      t.Month, t.Day, t.Hour, t.Minute, t.Second);
            if(t.TimeZone >= 0) {
        ...
        [&] {
            ...
            int recv_len =
                usb::cdc::driver->ReceiveSerial(buf.data(), send_len);
            while(recv_len == 0) {
                recv_len =
                    usb::cdc::driver->ReceiveSerial(buf.data(), send_len);
            }
    ...
    if(pipe_fd) {
        ...
        __asm__("sti");
        if(err) { Log(kWarn, "failed to wait finish: %s\n", err.Name()); }
        exit_code = ec;
    }
...
WithError<int> Terminal::ExecuteFile(fat::DirectoryEntry &file_entry,
    ...
    if(auto err = SetupPageMaps(args_frame_addr, 1)) { return {0, err}; }
    ...
    if(argc.error) { return {0, argc.error}; }
