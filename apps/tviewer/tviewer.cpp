...
void CopyUTF8String(char* dst, size_t dst_size,
	...
	while (*src) {
		...
		if (src + c > src_end || dst + c >= dst_end) { break; }
		memcpy(dst, src, c);
		src += c;
		dst += c;
	}
...
void DrawLines(const LinesType& lines, int start_line,
	...
	for (int i = 0; i < h; ++i) {
		...
		if (line_index < 0 || lines.size() <= line_index) { continue; }
...
extern "C" void main(int argc, char** argv) {
	...
	while (true) {
		...
		if (UpdateStartLine(&start_line, height, lines.size())) { break; }
	}
